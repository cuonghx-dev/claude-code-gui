...
const RULES: Array<{ test: (path: string) => boolean; invalidate: () => void }> = [
  ...
  { test: (p) => p.includes('/.claude/file-history/'), invalidate: () => queryClient.invalidateQueries({ queryKey: qk.checkpoints.all }) },
  { test: (p) => p.includes('/.claude/cli-history/'),  invalidate: () => queryClient.invalidateQueries({ queryKey: qk.terminals.all }) },
